...
client.on('message', async (receivedMessage) => {
    ...
    if (messageLower.startsWith(PREFIX)) {
        ...
        } else if (command === 'setbd') {
            try {
                Birthdays.find({ where: { date: commandArgs} })
                .on('success', function (birthdays) {
                    if (birthdays) {
                        const bdDBitem = birthdays.get(usernames);
                        const bdDBitemNew = bdDBitem.push(message.author.id);
                        birthdays.update({
                            usernames: bdDBitemNew
                        })
                        .then(function (err) {
                            const tag = await Birthdays.create({
                                date: commandArgs,
                ...
                if (e.name === 'SequalizeUniqueConstraintError') {
                    return message.reply('This should not have happened.');
                }
                return message.reply('Something went wrong with adding a tag.');
            }
        } else {
